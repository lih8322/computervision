...
정상 제품 이미지의 **평균(골든 이미지)** 을 기준으로, 입력이 기준에서 얼마나 벗어났는지를 계산해 표면 결함을 찾습니다. 불량은 종류가 무한하고 드물지만 정상은 확보하기 쉬우므로, **정상만 학습해 벗어난 것을 결함으로 판정**하는 이상탐지 방식입니다. 딥러닝이 아닌 고전 영상처리 기반 파이프라인입니다.
