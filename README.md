...
세 단계를 거치며 소프트웨어 처리만으로는 과검이 34.8 %에서 더 내려가지 않았습니다. 이 값은 **단일 골든 이미지 차분 방식의 구조적 한계**를 보여주며, 실무에서는 이 지점부터 광학 조건 개선 또는 학습 기반(딥러닝) 방법이 병행됩니다. 검사 방법마다 촬영 통제 의존도가 다르므로(차분 > 특징 기반 > 학습 기반), 환경에 맞춰 방법을 선택하게 된다는 것을 실험으로 확인했습니다.
