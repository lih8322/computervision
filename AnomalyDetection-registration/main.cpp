...
#include <opencv2/video/tracking.hpp>      // findTransformECC (ECC 정렬)
...
// ═══════════════════════════════════════════════════════════════════════════
//  결함 검출기 (v3 — ECC 정렬 + 적응형 임계값)
//
//  v2는 픽셀별 σ를 학습해 Z-score(=|입력−평균|/σ)로 판정했지만, 남은 과검이
//  34.8%에서 멈췄다. k를 올려도 안 줄어드는 것으로 보아, 원인이 무늬가 아니라
//  입력과 기준 사이의 미세한 위치·회전 어긋남(정렬 오차)일 가능성이 있다.
//
//  v3는 차분 전에 입력을 기준(mean)에 맞추는 단계를 추가한다.
//  ECC(Enhanced Correlation Coefficient, cv::findTransformECC)로 밝기 기반
//  정합을 구해 warp한 뒤, v2와 동일한 Z-score 판정을 적용한다.
// ═══════════════════════════════════════════════════════════════════════════
class DefectDetector {
public:
    struct Params {
        double k           = 3.0;   // 임계 배수: score > k 이면 결함
        double sigma_min   = 3.0;   // σ 바닥값 (0-나눗셈·과민 방지)
        int    blur_ksize  = 5;     // 가우시안 커널 (홀수)
        int    morph_ksize = 5;     // 모폴로지 커널
        int    min_area    = 50;    // 이보다 작은 덩어리는 노이즈로 버림
        // ── ECC 정렬 ──
        bool   use_ecc     = true;  // 정렬 on/off (v2와 비교용으로 끌 수 있음)
        int    ecc_iters   = 50;    // ECC 최대 반복
        double ecc_eps     = 1e-4;  // 수렴 조건(계수 변화량)
        double ecc_scale   = 0.25;  // 정렬 추정 해상도(1/4). 작을수록 빠르고 거칠음
    };
    Params params;
    ...
    std::vector<cv::Rect> detect(const cv::Mat& input) {
        ...
        // ★ v3 핵심: 입력을 기준(mean_)에 정렬(registration).
        //    미세한 위치·회전 어긋남을 맞춘 뒤 차분해야, 정렬 오차가 만드는
        //    경계선 과검을 줄일 수 있다. 실패하면 원본 그대로 진행(폴백).
...
private:
    // ── ECC 정렬: 입력(x, 32F)을 기준(mean_)에 맞춰 warp ──
    //  속도·안정성을 위해 1/ecc_scale 로 축소한 영상에서 아핀 변환을 추정하고,
    //  이동 성분만 배율로 되돌려 원본 해상도에 적용한다.
    //  (거친 해상도로 맞추면 결함이 정렬을 흔드는 영향도 줄어든다.)
    //  수렴 실패 시 예외 → 원본 그대로 반환(폴백).
    cv::Mat align_ecc(const cv::Mat& x) {
        const double s = params.ecc_scale;
        try {
            // 기준의 축소본은 한 번만 만들어 캐시(매 프레임 동일).
            if (mean_small_.empty())
                cv::resize(mean_, mean_small_, cv::Size(), s, s, cv::INTER_AREA);
            cv::Mat x_small;
            cv::resize(x, x_small, mean_small_.size(), 0, 0, cv::INTER_AREA);

            cv::Mat warp = cv::Mat::eye(2, 3, CV_32F);
            cv::TermCriteria crit(cv::TermCriteria::COUNT + cv::TermCriteria::EPS,
                                  params.ecc_iters, params.ecc_eps);
            cv::findTransformECC(mean_small_, x_small, warp, cv::MOTION_AFFINE, crit);

            // 축소본에서 구한 변환 → 원본 적용: 선형부(2x2)는 그대로,
            // 이동(마지막 열)만 1/s 배로 되돌린다.
            warp.at<float>(0, 2) /= static_cast<float>(s);
            warp.at<float>(1, 2) /= static_cast<float>(s);

            cv::Mat aligned;
            cv::warpAffine(x, aligned, warp, x.size(),
                           cv::INTER_LINEAR + cv::WARP_INVERSE_MAP,
                           cv::BORDER_REPLICATE);
            return aligned;
        } catch (const cv::Exception&) {
            return x;   // 정렬 실패 → 원본 유지
        }
    }
    ...
    cv::Mat  mean_small_;  // 평균의 축소본 (ECC 정렬용 캐시)
    cv::Size ref_size_;
};
...
int main(int argc, char** argv) {
    ...
    det.params.use_ecc     = (arg_i(7, det.params.use_ecc ? 1 : 0) != 0);
    ...
    std::cout << "파라미터: k=" << det.params.k
              ...
              << "  use_ecc=" << (det.params.use_ecc ? 1 : 0) << "\n";
